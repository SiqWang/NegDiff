...
class NegDiffusion:
    ...
    def neg_optimization_micro(self, latents, num_inner_steps, seed, epsilon):
        ...
        bar = tqdm(total=num_inner_steps * self.num_ddim_steps * num_microbatches)
        for m in range(num_microbatches):
            ...
            latent_safe = latent_mb.clone()
            latent_unsafe_list.append(latent_mb)
            latent_safe_list.append(latent_safe)
            # unsafe_uncond_emb_list.append([])
            
        for i in range(self.num_ddim_steps):
            unsafe_neg_emb = unsafe_neg_emb.clone().detach()
            unsafe_neg_emb.requires_grad = True
            optimizer = Adam([unsafe_neg_emb], lr=self.lr * (1. - i / 100.))
            ...
            for m in range(num_microbatches):
                for j in range(num_inner_steps):
                    ...
                    with torch.no_grad():
                        safe_noise_pred_cond = self.get_noise_pred_single(latent_safe_list[m], t, safe_cond_emb)
                        safe_noise_pred_uncond = self.get_noise_pred_single(latent_safe_list[m], t, safe_uncond_emb)
                        safe_noise_pred = safe_noise_pred_uncond + self.guidance_scale * (safe_noise_pred_cond - safe_noise_pred_uncond)
                        safe_latents_prev_rec = self.prev_step(safe_noise_pred, t, latent_safe_list[m])

                        unsafe_noise_pred_cond = self.get_noise_pred_single(latent_unsafe_list[m], t, unsafe_cond_emb)

                    unsafe_noise_pred_uncond = self.get_noise_pred_single(latent_unsafe_list[m], t, unsafe_neg_emb)
                    unsafe_noise_pred = unsafe_noise_pred_uncond + self.guidance_scale * (unsafe_noise_pred_cond - unsafe_noise_pred_uncond)
                    unsafe_latents_prev_rec = self.prev_step(unsafe_noise_pred, t, latent_unsafe_list[m])
                    ...
                    if micro_batch_size != 1:
                        ...
                        unsafe_neg_emb = unsafe_neg_emb_avg.unsqueeze(0).repeat(micro_batch_size,1,1)
                    
                    loss_item = loss.item()
                    # loss_list.append(loss_item)
                    # avg_item[m].append([])
                    # self.logger.info(f"loss for i={i}, m={m}, j={j}: {loss_item}")

                    # unsafe_neg_emb_avg = unsafe_neg_emb.mean(0)
                    # unsafe_neg_emb = unsafe_neg_emb_avg.unsqueeze(0).repeat(virtual_batch_size, 1, 1)                
                    bar.update()
                ...
                with torch.no_grad():
                    ...
                    latent_safe_list[m] = self.get_noise_pred(latent_safe_list[m], t, False, self.safe_context)
            
            unsafe_uncond_emb_list.append(unsafe_neg_emb.detach())

            # avg_loss = sum(loss_list) / len(loss_list)
            # avg_item[m].append(avg_loss)
        ...
        # return unsafe_uncond_emb_list, latent_mb, latent_safe
        return unsafe_uncond_emb_list, latent_unsafe_list, latent_safe_list
    

    def neg_optimization_micro_in(self, latents, num_inner_steps, epsilon):
        unsafe_neg_emb, unsafe_cond_emb = self.unsafe_context.chunk(2)
        safe_uncond_emb, safe_cond_emb = self.safe_context.chunk(2)
        ...
        latent_cur = self.prepare_latents(
            batch_size=micro_batch_size,  # single trajectory to save memory
            num_channels_latents=self.model.unet.config.in_channels,
            height=self.height, width=self.width,
            dtype=unsafe_cond_emb.dtype, device=self.model.device,
            generator=self.generator, latents=latents
        )
        latent_safe = latent_cur.clone()
        
        num_microbatches = virtual_batch_size // micro_batch_size
        ...
        for i in range(self.num_ddim_steps):
            ...
            for m in range(num_microbatches):
                neg_emb_to_optimize = unsafe_neg_emb.clone().detach()
                neg_emb_to_optimize.requires_grad = True
                optimizer = Adam([neg_emb_to_optimize], lr=5e-2 * (1. - i / 100.))
                ...
                for j in range(num_inner_steps):
                    ...
                    with torch.no_grad():
                        safe_noise_pred_cond = self.get_noise_pred_single(latent_mb, t, safe_cond_emb)
                        safe_noise_pred_uncond = self.get_noise_pred_single(latent_mb, t, safe_uncond_emb)
                        safe_noise_pred = safe_noise_pred_uncond + self.guidance_scale * (safe_noise_pred_cond - safe_noise_pred_uncond)
                        safe_latents_prev_rec = self.prev_step(safe_noise_pred, t, latent_mb)

                        unsafe_noise_pred_cond = self.get_noise_pred_single(latent_mb, t, unsafe_cond_emb)

                    unsafe_noise_pred_uncond = self.get_noise_pred_single(latent_mb, t, neg_emb_to_optimize)
                    unsafe_noise_pred = unsafe_noise_pred_uncond + self.guidance_scale * (unsafe_noise_pred_cond - unsafe_noise_pred_uncond)
                    unsafe_latents_prev_rec = self.prev_step(unsafe_noise_pred, t, latent_mb)
                    ...
                    neg_emb_avg = neg_emb_to_optimize.mean(0)
                    neg_emb_to_optimize.data = neg_emb_avg.unsqueeze(0).repeat(virtual_batch_size, 1, 1)
                    
                    loss_item = total_loss / num_microbatches
                
                    bar.update()
                    if loss_item < epsilon + i * 2e-5:
                        break
                    
                step_embedding.append(neg_emb_to_optimize.detach())
            ...
            averaged_emb = torch.stack(step_embedding).mean(dim=0)
            
            unsafe_neg_emb = averaged_emb
            unsafe_uncond_emb_list.append(averaged_emb)
    ...
    # def neg_optimization_avg(self, latents, num_inner_steps, epsilon, num_latents_to_average):
    #     # Initial context setup
    #     unsafe_neg_emb, unsafe_cond_emb = self.unsafe_context.chunk(2)
    #     safe_uncond_emb, safe_cond_emb = self.safe_context.chunk(2)

    #     unsafe_uncond_emb_list = []
    #     latent_cur_final, latent_safe_final = None, None

    #     # Total steps for the progress bar
    #     total_steps = self.num_ddim_steps * num_latents_to_average * num_inner_steps
    #     bar = tqdm(total=total_steps, desc="Optimizing Averaged Embedding")

    #     for i in range(self.num_ddim_steps):
    #         # A list to store the learned embeddings from each latent in this denoising step
    #         step_embeddings = []
            
    #         # NEW LOOP: Iterate to average over multiple latents
    #         for k in range(num_latents_to_average):
    #             # 1. Create a fresh, trainable copy of the unconditional embedding for this latent
    #             # It starts from the result of the previous denoising step's average
    #             neg_emb_to_optimize = unsafe_neg_emb.clone().detach()
    #             neg_emb_to_optimize.requires_grad = True
    #             optimizer = Adam([neg_emb_to_optimize], lr=1e-2 * (1. - i / 100.))

    #             # 2. Generate a new random latent for each iteration in the averaging loop
    #             # This ensures we are not overfitting to a single noise seed
    #             latent_cur = self.prepare_latents(
    ...
    #                 generator=self.generator, latents=None
    #             )
    #             latent_safe = latent_cur.clone()
                
    #             t = self.model.scheduler.timesteps[i]

    #             # Pre-calculate predictions for the 'safe' and conditional 'unsafe' parts
    #             with torch.no_grad():
    ...
    #             # 3. Inner optimization loop (same as before)
    #             for j in range(num_inner_steps):
    #                 unsafe_noise_pred_uncond = self.get_noise_pred_single(latent_cur, t, neg_emb_to_optimize)
    #                 unsafe_noise_pred = unsafe_noise_pred_uncond + self.guidance_scale * (unsafe_noise_pred_cond - unsafe_noise_pred_uncond)
    ...
    #                 # Averaging across the batch dimension (if batch_size > 1)
    #                 neg_emb_avg = neg_emb_to_optimize.mean(0)
    #                 neg_emb_to_optimize.data = neg_emb_avg.unsqueeze(0).repeat(self.batch_size, 1, 1)

    #                 loss_item = loss.item()
    #                 bar.update()
    #                 if loss_item < epsilon + i * 2e-5:
    #                     # Fill up the progress bar for the skipped steps
    #                     bar.update(num_inner_steps - (j + 1))
    #                     break

    #             # 4. Store the optimized embedding for this specific latent
    #             step_embeddings.append(neg_emb_to_optimize.detach())
                
    #             # Store the latents from the very last iteration to return as an example trajectory
    #             if i == self.num_ddim_steps - 1 and k == num_latents_to_average - 1:
    #                 with torch.no_grad():
    #                     unsafe_context = torch.cat([neg_emb_to_optimize, unsafe_cond_emb])
    #                     latent_cur_final = self.get_noise_pred(latent_cur, t, False, unsafe_context)
    #                     latent_safe_final = self.get_noise_pred(latent_safe, t, False, self.safe_context)


    #         # 5. Average the embeddings learned from all latents for the current denoising step
    #         averaged_emb = torch.stack(step_embeddings).mean(dim=0)
            
    #         # 6. Update the master unconditional embedding with the new average
    #         unsafe_neg_emb = averaged_emb
    #         unsafe_uncond_emb_list.append(averaged_emb)

    #     bar.close()
    #     return unsafe_uncond_emb_list, latent_cur_final, latent_safe_final
    ...
    def neg_generate(self, safe_prompt: str, unsafe_prompt: str, seed, num_inner_steps=50, early_stop_epsilon=1e-5, verbose=False):
        ...
        latents=None
        uncond_embeddings, latent_unsafe, latent_safe = self.neg_optimization_micro(latents, num_inner_steps, seed, early_stop_epsilon)
        ...
        for m in range(self.batch_size // self.micro_batch_size):
            ...
            image_safe.append(self.decode_latents(latent_safe[m]))
...
def main(args):
    ...
    unsafe_dir = os.path.join(args.output_dir, "with_negemb")
    ...
    for i, batch in enumerate(dataloader):
        ...
        for l in range(len(case_nums)):
            ...
            unsafe_uncond_embeddings, image_list, image_safe_list = neg_diff.neg_generate(safe_prompt, unsafe_prompt, seeds[l], args.num_inner_steps, verbose=True)
