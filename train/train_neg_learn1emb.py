...
class NegDiffusion:
    ...
    @torch.no_grad()
    def init_unsafe_prompt(self, unsafe_prompt: str, unsafe_uncond_input=None):
        if unsafe_uncond_input is None:
            ...
            uncond_embeddings = unsafe_uncond_input
            text_input = self.model.tokenizer(
                unsafe_prompt,
                padding="max_length",
                max_length=self.model.tokenizer.model_max_length,
            ...
            text_embeddings = self.model.text_encoder(text_input.input_ids.to(self.model.device))[0]
            self.unsafe_context = text_embeddings
            self.unsafe_prompt = unsafe_prompt
    ...
    def neg_optimization(self, latents, unsafe_uncond_emb, num_inner_steps, epsilon):
        if unsafe_uncond_emb is None:
            unsafe_neg_emb, unsafe_cond_emb = self.unsafe_context.chunk(2)
        else:
            unsafe_cond_emb = self.unsafe_context
        ...
        for i in range(self.num_ddim_steps):
            if unsafe_uncond_emb is not None:
                unsafe_neg_emb = unsafe_uncond_emb[i]
    ...
    def neg_generate(self, safe_prompt: str, unsafe_prompt: str, seed, unsafe_uncond_emb=None, num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
        ...
        self.init_unsafe_prompt(unsafe_prompt, unsafe_uncond_emb)
        ...
        latents=None
        uncond_embeddings, latent_unsafe, latent_safe = self.neg_optimization(latents, unsafe_uncond_emb, num_inner_steps, early_stop_epsilon)
...
def main(args):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if i != 0:
            # unsafe_uncond_emb = unsafe_uncond_emb_list[i-1]
            unsafe_uncond_emb = unsafe_uncond_emb_last
        else:
            unsafe_uncond_emb = None
        seeds = [int(s) for s in seeds]
        unsafe_uncond_embeddings, image_unsafe, image_safe = neg_diff.neg_generate(safe_prompts, unsafe_prompts, seeds, unsafe_uncond_emb=unsafe_uncond_emb, verbose=True)
        # unsafe_uncond_emb_list.append(unsafe_uncond_embeddings)
        unsafe_uncond_emb_last = unsafe_uncond_embeddings
        # print('image_safe len:', len(image_safe), 'image_unsafe len:', len(image_unsafe))
        ...
        for j in range(len(case_nums)):
            ...
            safe_prompt_path = os.path.join(safe_dir, f"{case_num}_{'-'.join(category)}.png")
            unsafe_prompt_path = os.path.join(unsafe_dir, f"{case_num}_{'-'.join(category)}.png")

            image_safe[j].save(safe_prompt_path)
            image_unsafe[j].save(unsafe_prompt_path)
            ...
            logger.info(
                f"Seed: {seed}, Iter: {i}, Case#: {case_num}\n"
    ...
    torch.save(unsafe_uncond_emb_last, f"{args.output_dir}/unsafe_uncond_emb_final.pt")
    logger.info(f"Saved unsafe_uncond_emb_final.")
