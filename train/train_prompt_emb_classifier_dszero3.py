...
class Eval:
    def __init__(self, args):
        ...
        if self.category == 'nudity':
            ...
            self.nude_model = self.nude_model.to(dtype=torch.float16)
    ...
    def __call__(self, samples_tensor, threshold=0.6):        
        ...
        """
        model_dtype = next(self.nude_model.parameters()).dtype  # should be torch.float32 now
        samples_tensor = samples_tensor.to(dtype=model_dtype)

        with autocast(dtype=model_dtype):
...
# --- new: a tiny trainable wrapper for your unsafe conditional embedding ---
class UnsafeCondParam(torch.nn.Module):
    def __init__(self, init_tensor: torch.Tensor):
        super().__init__()
        # register as Parameter so DS can shard/optimize it
...
class NegDiffusion(nn.Module):
    ...
    def neg_optimization_micro(self, latents, num_inner_steps, seed, epsilon, eval_func, logger):
        ...
        target_dtype = self.model.unet.dtype
        target_device = self.model.device

        # split unsafe context and cast
        self.unsafe_context = self.unsafe_context.to(dtype=target_dtype, device=target_device)
        unsafe_neg_emb, unsafe_cond_emb = self.unsafe_context.chunk(2)

        # unsafe_cond_emb = unsafe_cond_emb.detach().clone().requires_grad_(True)

        # turn into a Parameter owned by a tiny module
        trainable = UnsafeCondParam(unsafe_cond_emb.clone().detach().to(target_device, target_dtype))

        # IMPORTANT: require grads
        for p in trainable.parameters():
            p.requires_grad_(True)

        # Initialize DeepSpeed engine (only this module is trainable)
        # Choose an optimizer DS can shard; CPU offload will use DeepSpeedCPUAdam automatically if configured.
        # engine, optimizer, _, _ = deepspeed.initialize(
        #     model=trainable,
        #     # You can pass a DS optimizer config or create a torch.optim and pass here.
        #     model_parameters=trainable.parameters(),
        #     config=args.deepspeed  # path or dict; we loaded dict earlier, you can pass it too
        # )

        # # use engine for steps; replace "unsafe_cond_emb" usages with trainable(). 
        # # Retrieve a view to ensure autograd graph connects.
        # def get_unsafe_cond_emb():
        #     return engine.module().to(dtype=self.model.unet.dtype, device=self.model.device)
        try:
            import bitsandbytes.optim as bnb_optim
            optimizer = bnb_optim.Adam8bit(trainable.parameters(), lr=self.lr)
            logger.info("Using 8-bit Adam for embedding.")
        except ImportError:
            optimizer = torch.optim.Adam(trainable.parameters(), lr=self.lr)
            logger.info("Using standard Adam for embedding.")

        def get_unsafe_cond_emb():
            # always return the live Parameter tensor (correct dtype/device)
            return trainable().to(device=target_device, dtype=target_dtype)

        print("Trainable embedding dtype:", get_unsafe_cond_emb().dtype)
        ...
        # subset_steps = self.model.scheduler.timesteps[:: max(1, len(self.model.scheduler.timesteps)//20) ]
        ...
        for m in range(num_microbatches):
            for j in range(num_inner_steps):
                ...
                for t in self.model.scheduler.timesteps:
                    # t = t.to(self.model.unet.dtype)
                    context_for_grad = torch.cat([unsafe_neg_emb, get_unsafe_cond_emb()], dim=0)
                ...
                eval_dtype = next(eval_func.nude_model.parameters()).dtype
                eval_device = next(eval_func.nude_model.parameters()).device
                image_tensor_for_grad = image_tensor_for_grad.to(device=eval_device, dtype=eval_dtype).contiguous()
                ...
                if micro_batch_size != 1:
                    with torch.no_grad():
                        unsafe_cond_emb_avg = get_unsafe_cond_emb().mean(0)
                        new_val = unsafe_cond_emb_avg.unsqueeze(0).repeat(micro_batch_size, 1, 1)
                        trainable.param.copy_(new_val)
                    # unsafe_cond_emb_avg = get_unsafe_cond_emb().mean(0)
                    # # re-write the Parameter values in-place with averaged copy
                    # with torch.no_grad():
                    #     new_val = unsafe_cond_emb_avg.unsqueeze(0).repeat(micro_batch_size, 1, 1)
                    #     engine.module().copy_(new_val)  # engine.module() returns the Parameter tensor
                ...
                score = float(unsafe_preds_tensor.mean().item())
                bar.set_postfix({'loss': f'{loss_item:.4f}'})
                logger.info(f"Step {j+1}, Loss: {loss_item:.6f}, Score: {score:.4f}")
                bar.update()
                torch.cuda.empty_cache()
        ...
        final_optimized_emb = trainable.param.detach().clone().to(self.model.device, dtype=target_dtype)

        # final_optimized_emb = unsafe_cond_emb.detach().clone()
        # final_optimized_emb = engine.module().detach().clone()  # the trained embedding
...
def main(args):
    ...
    # read ds config (string path or dict both ok)
    def load_ds_config(path):
        import json
        with open(path) as f:
            return json.load(f)

    ds_config = load_ds_config(args.deepspeed)  # args.deepspeed="ds_zero3.json"

    # zero_init_ctx = deepspeed.zero.Init(config_dict_or_path=ds_config, dtype=torch.bfloat16) \
    #                 if ds_config.get("bf16", {}).get("enabled", False) else \
    #                 deepspeed.zero.Init(config_dict_or_path=ds_config)

    # shard the heavy modules at creation time
    # with zero_init_ctx:
    ...
    unet = UNet2DConditionModel.from_pretrained(args.model_id, subfolder="unet").to(args.device)
    vae = AutoencoderKL.from_pretrained(args.model_id, subfolder="vae").to(args.device)
    text_encoder = CLIPTextModel.from_pretrained(args.model_id, subfolder="text_encoder").to(args.device)

    tokenizer = CLIPTokenizer.from_pretrained(args.model_id, subfolder="tokenizer")
    
    safety_checker = StableDiffusionSafetyChecker.from_pretrained(
        args.model_id, subfolder="safety_checker"
    )
    ...
    # Build/patch your existing pipeline
    ldm_stable = ModifiedStableDiffusionPipeline(
        vae=vae,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--deepspeed", type=str, default="ds_zero3.json", help="Path to DeepSpeed config file.")
