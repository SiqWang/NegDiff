...
class NegDiffusion:
    ...
    def neg_optimization_micro(self, num_inner_steps, seed, epsilon):
        unsafe_neg_emb, unsafe_cond_emb = self.unsafe_context.chunk(2)
        safe_uncond_emb, safe_cond_emb = self.safe_context.chunk(2)
        
        # virtual_batch_size = self.batch_size
        micro_batch_size = self.micro_batch_size # 4
        num_microbatches = self.batch_size // self.micro_batch_size # 16//4 = 4
        latent_unsafe_list = []
        latent_safe_list = []
        unsafe_uncond_emb_list = []
        ...
        for m in range(num_microbatches):
            latent_mb = self.prepare_latents(
                ...
                generator=[torch.Generator(device=self.device).manual_seed(
                    int(s) + (num_microbatches - m - 1) * 100
                    ) for s in seed]
            )
    ...
    def neg_generate(self, safe_prompt: str, unsafe_prompt: str, seed, num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
        ...
        uncond_embeddings, latent_unsafe, latent_safe = self.neg_optimization_micro(num_inner_steps, seed, early_stop_epsilon)
...
def main(args):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # logger.info(f"Seed: {int(seed)}, Iter: {i}, Case#: {case_num}: \nSafe Prompt: {safe_prompt[0]}.\nUnsafe Prompt: {unsafe_prompt[0]}")
 
        seeds = [int(s) for s in seeds]
        
        # import pdb
        # pdb.set_trace()
        unsafe_uncond_embeddings, image_list, image_safe_list = neg_diff.neg_generate(safe_prompts, unsafe_prompts, seeds, args.num_inner_steps, verbose=True)
        ...
        torch.save(unsafe_uncond_embeddings, f"{args.output_dir}/unsafe_uncond_emb_iter_{i}.pt")
        ...
        for g in range(num_microbatches):
            ...
            generator = [torch.Generator(device=args.device).manual_seed(
                s + (num_microbatches - g - 1) * 100) for s in seeds]

            image_ori = ldm_stable(
                            prompt=list(unsafe_prompts),
            ...
            for j in range(len(case_nums)):
                case_num = int(case_nums[j])
                seed = int(seeds[j])
                safe_prompt = safe_prompts[j]
                unsafe_prompt = unsafe_prompts[j]
                ...
                # Save images (now indexed per sample)
                image_safe[j].save(os.path.join(safe_dir, f"{case_num}_{'-'.join(category)}_{g}.png"))
                image_unsafe[j].save(os.path.join(unsafe_dir, f"{case_num}_{'-'.join(category)}_{g}.png"))
                image_ori[j].save(os.path.join(ori_dir, f"{case_num}_{'-'.join(category)}_{g}.png"))

                # Evaluate nudity
                if safe_is_unsafe_list[j]:
                    unsafe_cnt += 1
                else:
                    safe_cnt += 1
                if unsafe_is_unsafe_list[j]:
                    un_unsafe_cnt += 1
                else:
                    un_safe_cnt += 1
                if ori_is_unsafe_list[j]:
                    ori_unsafe_cnt += 1
                else:
                    ori_safe_cnt += 1
                    
                logger.info(
                    f"Seed: {seed}, Iter: {i}, Micro-batch: {g}, Case#: {case_num}\n"
                    f"Safe Prompt: {safe_prompt}\nUnsafe Prompt: {unsafe_prompt}"
                )
                logger.info(f"The image generated by safe prompt is unsafe: {safe_is_unsafe_list[j]}, toxicity pred: {safe_preds_list[j]:.3f}")
                logger.info(f"The image generated by unsafe prompt is unsafe: {unsafe_is_unsafe_list[j]}, toxicity pred: {unsafe_preds_list[j]:.3f}")
                logger.info(f"Original unsafe prompt is unsafe: {ori_is_unsafe_list[j]}, toxicity pred: {ori_preds_list[j]:.3f}")
